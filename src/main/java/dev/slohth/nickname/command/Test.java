...
public class Test {
    ...
    public void testCommand(Args cmd) {
        Player player = cmd.getPlayer();
        if (player.hasPermission("tab.sort.member")) player.sendMessage(ChatColor.AQUA + "member");
        ...
        if (player.hasPermission("rocketplaceholder.member")) player.sendMessage(ChatColor.DARK_AQUA + "member");
